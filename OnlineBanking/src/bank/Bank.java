		...
		if (allAccounts.containsKey(initiatorIban)) {
			if (allAccounts.get(initiatorIban).getAmount() >= amount) {
				if (allAccounts.containsKey(recipientIban)) {
					...
					System.out
							.println("There's no account with this IBAN in the bank!"); // for
																						// testing
					return false;
				}
			} else {
				System.out
						.println("You have not enough money for this transaction!"); // for
																						// testing
				return false;
			}
		} else {
			System.out
					.println("You don't have an account with this IBAN! Please enter a valid IBAN!");
			return false;
		}
		...
		if (allAccounts.containsKey(initiatorIban)) {
			if (allAccounts.get(initiatorIban).getAmount() >= amount) {
				if (!client.getClientAccounts().containsKey("CurrentAccount")) {
					client.getClientAccounts().put("CurrentAccount",
							new HashMap<String, ArrayList<Account>>());
				}
				if (!client.getClientAccounts().get("CurrentAccount")
						.containsKey("CurrentAccountBGN")) {
					client.getClientAccounts().get("CurrentAccount")
							.put("CurrentAccountBGN", new ArrayList<Account>());
				}
				client.getClientAccounts().get("CurrentAccount")
						.get("CurrentAccountBGN")
						.add(new CurrentAccountBGN(amount));
				allAccounts.get(initiatorIban).removeMoneyFromAccount(amount);
			} else {
				System.out
						.println("You have not enough money to open this Account!"); // for
																						// testing
			}
		} else {
			System.out
